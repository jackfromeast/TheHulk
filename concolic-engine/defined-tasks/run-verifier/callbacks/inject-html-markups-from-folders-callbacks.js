 ...
 * @param {*} page
 */
async function injectHTMLMarkupsOnceCallbacks(visitor, page) {
  ...
  if (visitor.exploits.length===0 && !visitor.verifedURLs[visitor.curURL]) {
    ...
    const exploitInputPath = path.resolve(exploitInputFolder, 'exploit-short.txt');
    ...
    visitor.exploits = exploitInputs.join('\n');
    visitor.retestCurURL = true;
    visitor.retestMaxTimes = visitor.exploits.length;
  }

  
  visitor.logger.debug(`Injecting HTML markups to the page: ${visitor.exploits}`);
