...
function generateHTML(name, scripts, testCode) {
  const scriptTags = scripts.map(script => {
    const typeAttribute = script.type === 'module' ? ' type="module"' : '';
    return `<script src="${script.path}"${typeAttribute}></script>`;
  }).join('\n');
...
(function main() {
  ...
  testModules.forEach(module => {
    ...
    jsTestNames.forEach(jsTestName => {
      ...
      const typeMatch = jsFileContent.match(/@Type: (.+)/);
      ...
      if (importFlag) {
        mainTestFiles.push({path: `./${module}/scripts/${file}`, type: type});
    ...
    if (mainTestFiles.length > 0) {
      const htmlFileName = `${module}.html`;
      const htmlFilePath = path.join(testPagesDir, htmlFileName);
      const { html } = generateHTML(module, mainTestFiles, testCode);

      fs.writeFileSync(htmlFilePath, html, 'utf8');
      console.log(`[+] Generated ${htmlFilePath}`);

      pages.push({ path: htmlFileName, name: module, category: module });
    } else {
